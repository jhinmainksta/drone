...
// фишка получается в том, что при первом открытии собирается лист ссылок,
// которые добавляют при нажатии параметры в url, которые сидят в window.location.search
// и таким образом с помощью парамс мы получаем инфу о том, что мы на страничке какого-то топика
...
function viewTopicsList() {
	...
	ros.getTopics(function (topics) {
		topicsList.innerHTML = topics.topics
	...
	new ROSLIB.Topic({ ros: ros, name: topic }).subscribe(function (msg) {
		...
		if (msg.header && msg.header.stamp) {
			if (params.date || params.offset) {
				let date = new Date(
					msg.header.stamp.secs * 1e3 + msg.header.stamp.nsecs * 1e-6
				);
...
topicMessage.addEventListener("mousedown", function () {
	mouseDown = true;
});
topicMessage.addEventListener("mouseup", function () {
	mouseDown = false;
});
