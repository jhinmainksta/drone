...
> **Hint** Alternatively we recommend to use the [ZeroTier](zerotier_vpn.md) VPN-service.
...
> **Info** A Raspberry Pi camera device `/dev/video0` could be used by `drone` systemd service. In this case `gst-rtsp-launch` will not get an access to the device. For stop the `drone` run command `sudo systemctl stop drone`.
